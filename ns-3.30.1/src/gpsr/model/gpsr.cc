...
#define JITTER (Seconds (UniformRandomVariable(-GPSR_MAXJITTER, GPSR_MAXJITTER).GetValue())) 
#define FIRST_JITTER (Seconds (UniformRandomVariable(0, GPSR_MAXJITTER).GetValue())) //first Hello can not be in the past, used only on SetIpv4
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  a->SetAttribute("Min", DoubleValue(0 ));
  a->SetAttribute("Max", DoubleValue(GPSR_MAXJITTER));

  HelloIntervalTimer.SetFunction (&RoutingProtocol::HelloTimerExpire, this);
  HelloIntervalTimer.Schedule (Seconds(a->GetValue()));
...
{
  Ptr<UniformRandomVariable> a = CreateObject<UniformRandomVariable> ();
  a->SetAttribute("Min", DoubleValue(-GPSR_MAXJITTER ));
  a->SetAttribute("Max", DoubleValue(GPSR_MAXJITTER));
  SendHello ();
  HelloIntervalTimer.Cancel ();
  HelloIntervalTimer.Schedule (HelloInterval + Seconds(a->GetValue()));
}
