...
void
RoutingStats::IncDropPkts ()
{
  m_DropPkts ++;
}

uint32_t
RoutingStats::GetDropPkts ()
{
  return m_DropPkts;
}
...
public:
  ...
  void OnOffDropTrace (std::string context, Ptr<const Packet> packet);
...
RoutingHelper::SetupRoutingProtocol (NodeContainer & c)
{
  ...
  Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("vanet-routing-compare.routes", std::ios::out);
  ...
  switch (m_protocol)
    ...
    case 5:
      if(m_routingTables != 0)
      {
        gpsr.PrintRoutingTableAllEvery(Seconds(2), routingStream);
      }
      m_protocolName = "GPSR";
      break;
    default:
...
void
RoutingHelper::OnOffDropTrace (std::string conext, Ptr<const Packet> packet)
{
  //std::ostringstream oss;

  //oss << Simulator::Now ().GetSeconds () << " " << Simulator::Now().
  routingStats.IncDropPkts ();
}
...
VanetRoutingExperiment::ConfigureApplications ()
{
  ...
  Config::Connect ("/NodeList/*/ApplicationList/*/$ns3::OnOffApplication/Drop", MakeCallback (&RoutingHelper::OnOffDropTrace, m_routingHelper));
}
...
VanetRoutingExperiment::Run ()
{
  ...
  monitor->SetAttribute("MaxPerHopDelay", TimeValue(MilliSeconds(1.0)));
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>:: const_iterator i = stats.begin(); i != stats.end(); ++i)
  {
    ...
    if(i->second.rxPackets != 0){
      hopCounts += (i->second.timesForwarded / i->second.rxPackets + 1);
  ...
  uint32_t RxPackets = m_routingHelper->GetRoutingStats().GetCumulativeRxPkts();
  uint32_t TxPackets = m_routingHelper->GetRoutingStats().GetCumulativeTxPkts();
  uint32_t DropPackets = m_routingHelper -> GetRoutingStats(). GetDropPkts();
  
  if(count != 0)
  {
    std::cout << std::fixed << std::setprecision(2) << "Tx Packets(FlowMonitor)" << totalTx << std::endl;
    std::cout << std::fixed << std::setprecision(2) << "Rx Packets(FlowMonitor)" << totalRx << std::endl;
    std::cout << std::fixed << std::setprecision(2) << "Average hops per Flow(FlowMonitor)" << hopCounts/count << std::endl;
    std::cout << std::fixed << std::setprecision(2) << "Average delay per Flow(FlowMonitor)" << delay/count << std::endl;
    std::cout << std::fixed << std::setprecision(2) << "Tx Packets(RoutingHelper)" << TxPackets << std::endl;
    std::cout << std::fixed << std::setprecision(2) << "Rx Packets(RoutingHelper)" << RxPackets << std::endl;
    std::cout << std::fixed << std::setprecision(2) << "Drop Packets(Routing Helper)" << DropPackets << std::endl;
  }
